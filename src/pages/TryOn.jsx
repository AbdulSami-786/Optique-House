...
// // ── Resolve initial frame from router state ───────────────────────────────────
// const resolveInitialFrame = (state) => {
//   console.log('resolveInitialFrame called with state:', state);
  
//   if (!state?.frameId) {
//     console.log('No frameId in state, using default:', GLASS_OPTIONS[0].id);
//     return GLASS_OPTIONS[0].id;
//   }
  
//   const match = GLASS_OPTIONS.find(g => g.id === state.frameId);
  
//   console.log('Looking for frame:', {
//     requestedFrameId: state.frameId,
//     foundMatch: match?.id,
...
//   console.log('TryOn component mounted with location.state:', location.state);

//   const initialFrameId      = resolveInitialFrame(location.state);
//   const incomingProductName = location.state?.productName || null;

//   console.log('Initial frame ID selected:', initialFrameId);
...
//     const onResize = () => {
//       const m = getIsMobile();
//       if (isMobileRef.current === m) return;
...
//     cachedGlassRef.current = GLASS_OPTIONS.find(g => g.id===glasses) || null;
//     setAdjUIState({...(adjustmentsRef.current[glasses] || DEFAULT_ADJ)});
//     console.log('Glasses changed to:', glasses);
//   }, [glasses]);
...
// ── Code → frame mapping ──────────────────────────────────────────────────────
// This MUST match ProductCard.jsx CODE_TO_FRAME exactly
const CODE_TO_FRAME = {
  "T04917":  "[/glass37.png, /glass16.png]",
  "K01":     "/glass42.png",
  "D403054S":"/glass41.png",
  "20237":   "/glass14.png",
  "RB4455F": "/glass35.png",
  "5013":    "/glass7.png",
  "S2160":   "/glass33.png",
...
// ── Resolve initial frame from URL search params ───────────────────────────────
// URL format: /tryon?code=T04917&name=Product+Name
const resolveInitialFrame = (searchParams) => {
  const code = searchParams.get("code");
  if (code && CODE_TO_FRAME[code]) {
    const frameId = CODE_TO_FRAME[code];
    // Verify it exists in GLASS_OPTIONS
    const match = GLASS_OPTIONS.find(g => g.id === frameId);
    if (match) return match.id;
  }
  // Fallback: default to first frame
  return GLASS_OPTIONS[0].id;
};
...
const TryOn = () => {
  ...
  // ✅ KEY FIX: Read ?code= from URL search params
  const [searchParams] = useSearchParams();
  const incomingProductName = searchParams.get("name") || null;

  // Resolve which frame to show based on URL ?code= param
  const initialFrameId = resolveInitialFrame(searchParams);
      ...
      <>
        ...
        >
          <div style={{ position:"relative", flex:"1 1 auto", minHeight:0, background:"#000", overflow:"hidden" }}>
            ...
            <canvas ref={canvasRef} width={480} height={360} aria-label="AR glasses try-on" style={{ position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"contain", display:"block" }} />
            ...
            {cameraReady && (
              <div style={{ position:"absolute", top:12, right:12, zIndex:20, display:"flex", alignItems:"center", background:"rgba(0,0,0,0.55)", ...glassPill, border:`1px solid ${faceTracking?"rgba(34,197,94,.35)":"rgba(115,165,202,.30)"}`, padding:"5px 12px" }}>
                <span className={`ar-dot ${faceTracking?"ar-dot--green":""}`} />
                <span style={{ fontSize:10, fontWeight:600, color:"rgba(255,255,255,.80)", letterSpacing:".5px" }}>{faceTracking ? "Tracking" : "Searching..."}</span>
              </div>
            )}

            {cameraReady && currentGlass && (
              <div aria-live="polite" style={{ position:"absolute", bottom:16, left:"50%", transform:"translateX(-50%)", zIndex:20, whiteSpace:"nowrap", background:"rgba(0,0,0,0.55)", ...glassPill, border:"1px solid rgba(232,127,36,.35)", padding:"7px 20px", display:"flex", alignItems:"center", gap:10 }}>
                <span style={{ fontSize:12, fontWeight:700, color:"rgba(254,253,223,.95)" }}>{currentGlass.name}</span>
                <span style={{ width:1, height:11, background:"rgba(232,127,36,.40)", display:"inline-block" }} />
            ...
            {cameraReady && (
              <div style={{ position:"absolute", bottom:56, left:"50%", transform:"translateX(-50%)", display:"flex", gap:4, zIndex:20 }}>
                {GLASS_OPTIONS.map((g,i) => (
                  <div key={g.id} style={{ width:i===idx?14:4, height:4, borderRadius:3, background:i===idx?"#E87F24":"rgba(255,255,255,.25)", transition:"all .25s ease" }} />
                ))}
              </div>
            )}

            {!cameraReady && (
              <div role="status" style={{ position:"absolute", inset:0, zIndex:50, background:"radial-gradient(ellipse 120% 80% at 55% 30%,rgba(232,127,36,.08),rgba(0,0,0,.98) 60%)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:24 }}>
                <div style={{ position:"relative", width:44, height:44 }}>
                  <div className="spinner"/><div className="spinner-inner"/>
                </div>
                <div style={{ textAlign:"center" }}>
                  <div style={{ fontFamily:"'Outfit',sans-serif", fontSize:22, fontWeight:800, marginBottom:6, background:"linear-gradient(135deg,#F5A623,#E87F24)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>VR.OPTICS</div>
                  <div style={{ fontSize:9, fontWeight:700, letterSpacing:"3px", color:"#E87F24", marginBottom:8 }}>INITIALIZING</div>
                  <div style={{ fontSize:10, color:"rgba(254,253,223,.40)" }}>Allow camera access to continue</div>
                </div>
          ...
          <div style={{ flexShrink:0, paddingBottom:"env(safe-area-inset-bottom,10px)", background:"linear-gradient(to top,rgba(8,4,1,.98) 40%,rgba(8,4,1,.80) 70%,transparent 100%)" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 20px 2px" }}>
              ...
              {GLASS_OPTIONS.map((g) => {
                ...
                return (
                  <div key={g.id} ref={isA ? activeCarouselRef : null}
                    ...
                    onKeyDown={e => (e.key==="Enter"||e.key===" ") && setGlasses(g.id)}
                    style={{ minWidth:60 }}>
                    <div className="lens-ring__circle" style={{ width:sz, height:sz, background:isA?"radial-gradient(circle at 35% 35%,rgba(232,127,36,.28),rgba(10,5,2,.95))":"radial-gradient(circle at 35% 35%,rgba(255,255,255,.07),rgba(10,5,2,.85))" }}>
                      <div className="lens-ring__glow-ring" />
                      <img src={g.id} alt={g.name} loading="lazy" className="lens-ring__img" />
                    </div>
                    <span className="lens-ring__label">{g.name}</span>
  ...
  return (
    <>
      <div style={{ fontFamily:"'Space Grotesk',sans-serif", background:C.gradBg, color:C.text, height:"100vh", display:"flex", overflow:"hidden" }}>
        ...
        <div aria-hidden style={{ position:"fixed", inset:0, pointerEvents:"none", zIndex:0 }}>
          <div style={{ position:"absolute", top:"-15%", right:"-8%", width:"52vw", height:"52vw", borderRadius:"50%", background:"radial-gradient(circle,rgba(232,127,36,.14) 0%,transparent 70%)" }} />
          <div style={{ position:"absolute", bottom:"-20%", left:"-12%", width:"48vw", height:"48vw", borderRadius:"50%", background:"radial-gradient(circle,rgba(115,165,202,.12) 0%,transparent 70%)" }} />
        </div>

        {/* ── Camera (75%) ── */}
        <div style={{ position:"relative", zIndex:1, flex:"0 0 75%", maxWidth:"75%", padding:20, display:"flex", flexDirection:"column" }}>
          <div style={{ flex:1, position:"relative", borderRadius:22, overflow:"hidden", border:`1px solid ${C.glassBorder}`, background:"#000", boxShadow:"inset 0 0 60px rgba(0,0,0,.40),0 8px 40px rgba(30,41,59,.12)" }}>
            ...
            {/* Show product name from URL param */}
            {incomingProductName && cameraReady && (
              <div style={{ position:"absolute", bottom:60, left:"50%", transform:"translateX(-50%)", display:"flex", alignItems:"center", gap:10, padding:"7px 20px", borderRadius:999, background:"rgba(0,0,0,.52)", border:"0.5px solid rgba(232,127,36,.35)", whiteSpace:"nowrap", zIndex:6, pointerEvents:"none", animation:"fadeIn .35s ease" }}>
            ...
            {cameraReady && (
              <div role="status" style={{ position:"absolute", top:16, right:16, zIndex:5, display:"flex", alignItems:"center", background:"rgba(0,0,0,.42)", ...glassPill, border:`1px solid ${faceTracking?"rgba(34,197,94,.28)":"rgba(115,165,202,.28)"}`, padding:"5px 14px", animation:"fadeIn .3s ease" }}>
                <span className={`ar-dot ${faceTracking?"ar-dot--green":""}`} />
                <span style={{ fontSize:10, fontWeight:600, color:"rgba(255,255,255,.80)", letterSpacing:".5px" }}>{faceTracking ? "Face Tracking Active" : "Searching for Face..."}</span>
              </div>
            )}

            <div style={{ position:"absolute", bottom:16, left:16, zIndex:5 }}>
              <div aria-live="polite" style={{ background:"rgba(0,0,0,.52)", ...glassPill, border:`0.5px solid ${C.primary25}`, padding:"8px 20px", display:"flex", alignItems:"center", gap:12 }}>
                <span style={{ fontSize:9, fontWeight:700, color:"rgba(254,253,223,.50)", letterSpacing:"1.5px" }}>SELECTED</span>
                <span style={{ width:1, height:11, background:C.primary30, display:"inline-block" }} />
                <span style={{ fontSize:13, fontWeight:700, color:"rgba(254,253,223,.95)" }}>{currentGlass?.name}</span>
            ...
            <video ref={videoRef} style={{ position:"absolute", left:"-100%", top:"-100%", width:"1px", height:"1px", opacity:0, pointerEvents:"none" }} autoPlay playsInline muted />
            <canvas ref={canvasRef} width={480} height={360} aria-label="AR glasses try-on camera view" style={{ display:"block", width:"100%", height:"100%", objectFit:"cover" }} />

            {!cameraReady && (
              <div role="status" style={{ position:"absolute", inset:0, borderRadius:22, zIndex:30, background:`radial-gradient(ellipse 100% 60% at 55% 30%,rgba(232,127,36,.08),rgba(254,253,223,.97) 55%)`, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:28 }}>
                <div style={{ position:"relative", width:50, height:50 }}>
                  <div className="spinner"/><div className="spinner-inner"/>
                </div>
                <div style={{ textAlign:"center" }}>
        ...
        {/* ── Controls panel (25%) ── */}
        <div className="right-panel" role="complementary"
          style={{ position:"relative", zIndex:1, flex:"0 0 25%", maxWidth:"25%", overflowY:"auto", padding:"20px 16px 20px 4px", display:"flex", flexDirection:"column", gap:12, borderLeft:`1px solid ${C.glassBorder}`, background:"linear-gradient(180deg,rgba(245,243,199,.60),rgba(254,253,223,.80))", backdropFilter:"blur(20px)", WebkitBackdropFilter:"blur(20px)" }}>
            ...
            {GLASS_OPTIONS.map(g => {
              ...
              return (
                <div key={g.id} ref={isA ? activeGridRef : null}
                  ...
                  onClick={() => setGlasses(g.id)}
                  onKeyDown={e => (e.key==="Enter"||e.key===" ") && setGlasses(g.id)}
                  style={{ borderRadius:14, background:isA?C.primary12:"rgba(254,253,223,.55)", border:`1px solid ${isA?C.primary:C.surfaceBorder}`, padding:"10px 6px", display:"flex", flexDirection:"column", alignItems:"center", gap:5, cursor:"pointer", boxShadow:isA?`0 0 20px rgba(232,127,36,.20),0 4px 12px rgba(30,41,59,.08)`:`0 1px 4px ${C.text06}`, transition:"all .22s cubic-bezier(.22,1,.36,1)", backdropFilter:"blur(8px)", WebkitBackdropFilter:"blur(8px)" }}>
                  <div style={{ width:"100%", height:48, display:"flex", alignItems:"center", justifyContent:"center", borderRadius:10, overflow:"hidden", background:isA?C.primary12:C.text06 }}>
                    <img src={g.id} alt={g.name} loading="lazy" crossOrigin="anonymous" style={{ width:"90%", height:"90%", objectFit:"contain", filter:isA?"drop-shadow(0 0 5px rgba(232,127,36,.55))":"brightness(.80) saturate(.75)", transition:"filter .2s" }} />
                  </div>
          ...
          <Section title="FRAME CALIBRATION" icon="⚙️">
            <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:14 }}>
              <button onClick={resetAdj} style={{ fontSize:9, fontWeight:700, color:C.primary, background:C.primary12, border:`0.5px solid ${C.primary25}`, padding:"5px 14px", borderRadius:100, cursor:"pointer" }}>Reset</button>
            </div>
            <SliderRow label="WIDTH"    value={curAdj.scaleW}  min={0.3}  max={3}   step={0.05} onChange={v=>setAdj("scaleW",v)}  fmt={v=>`${v.toFixed(2)}×`} />
            <SliderRow label="HEIGHT"   value={curAdj.scaleH}  min={0.3}  max={3}   step={0.05} onChange={v=>setAdj("scaleH",v)}  fmt={v=>`${v.toFixed(2)}×`} />
            <SliderRow label="MOVE L/R" value={curAdj.offsetX} min={-150} max={150} step={1}    onChange={v=>setAdj("offsetX",v)} fmt={v=>`${v>0?"+":""}${v}px`} />
